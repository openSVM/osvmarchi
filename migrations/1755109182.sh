...
# Users can still opt-in to Cloudflare DNS using: osvmarchi-setup-dns cloudflare
...
if [ -f /etc/systemd/resolved.conf ]; then
  ...
  if [ -f /etc/systemd/network/99-osvmarchi-dns.network ]; then
    sudo rm -f /etc/systemd/network/99-osvmarchi-dns.network
  ...
  echo "To use Cloudflare DNS, run: osvmarchi-setup-dns Cloudflare"
fi
